...
def _seed_permissions(db: Session) -> None:
    permission_names = [
        ...
        "menu.sales.prefactura.mobile",
        "menu.sales.mis_ventas.mobile",
        "menu.sales.mis_comisiones.mobile",
        "menu.inventory",
        "menu.inventory.caliente",
        "menu.inventory.ingresos",
        "menu.inventory.egresos",
        "menu.inventory.requisas",
        "menu.inventory.preproduction.mobile",
        "menu.finance",
        "menu.accounting",
        "menu.reports",
        "menu.data",
        ...
        "access.sales.prefactura.mobile",
        "access.sales.mis_ventas.mobile",
        "access.sales.mis_comisiones.mobile",
        "access.inventory",
        "access.inventory.caliente",
        "access.inventory.ingresos",
        "access.inventory.egresos",
        "access.inventory.requisas",
        "access.inventory.preproduction.mobile",
...
def init_db() -> None:
    ...
    if "regalias_productos" in inspector.get_table_names():
        columns = {column["name"] for column in inspector.get_columns("regalias_productos")}
        if "cantidad_total" not in columns:
            with engine.begin() as conn:
                conn.execute(text("ALTER TABLE regalias_productos ADD COLUMN cantidad_total NUMERIC(14, 2) NOT NULL DEFAULT 0"))
        if "modo_asignacion" not in columns:
            with engine.begin() as conn:
                conn.execute(text("ALTER TABLE regalias_productos ADD COLUMN modo_asignacion VARCHAR(20) NOT NULL DEFAULT 'LIBRE'"))
